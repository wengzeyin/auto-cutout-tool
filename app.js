...
function buildMatteProfile(base, imageType = "unknown") {
  ...
  if (type === "illustration" || type === "line-art" || type === "sticker") {
    ...
    profile.postEdgeCoreNeighborThreshold = 224;
    profile.postEdgeCoreNeighborCount = profile.fidelity === "preserve" ? 6 : 5;
    profile.postEdgeCoreNormalizePasses = 2;
  }
...
function splitSparseStrongSeedComponent(component, imageData, coreChildren, minCoreArea, pad, settings) {
  ...
  const children = seeds
    .map((seed, index) => {
      const padded = padBox({ minX: seed.x, minY: seed.y, maxX: seed.x + seed.width - 1, maxY: seed.y + seed.height - 1 }, splitPad, width, height);
      return measureBoxAsComponent({
        id: index + 1,
        x: padded.minX,
        y: padded.minY,
...
function splitRepeatedRowComponent(component, imageData, coreMask, minCoreArea, pad, settings) {
  ...
  for (let y = startY; y < endY; y += 1) {
    for (let x = startX; x < endX; x += 1) {
      if (coreMask[y * width + x]) projection[x - startX] += 1;
    }
  }
...
function absorbTinyMultiObjectFragments(components, imageData, minArea, settings = getSplitSettings()) {
