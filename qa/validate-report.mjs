...
for (const row of rows) {
  ...
  if (isSvgScenario(scenario)) {
    ...
    if (Number(metrics.svgCubicHandleOutlierRatio || 0) > 0.1) failures.push(`${name}: svgCubicHandleOutlierRatio ${format(metrics.svgCubicHandleOutlierRatio)} is too high.`);
  }
...
function requiredMetricFieldsForScenario(scenario) {
  ...
  if (isSvgScenario(scenario)) fields.push("svgCommandCount", "svgCommandDensity", "svgFractionalCoordinateRatio", "svgCubicHandleOutlierRatio", "svgBlockyRisk");
