...
const cases = [
  ...
  {
    name: "micro-badges-between-large-stickers",
    draw: (img) => {
      for (const cx of [170, 390, 610]) {
        ellipse(img, cx, 190, 58, 70, 255);
        rect(img, cx - 42, 248, 84, 38, 245);
      }
      rect(img, 278, 176, 6, 6, 255);
      rect(img, 498, 176, 6, 6, 255);
      rect(img, 248, 178, 66, 3, 30);
      rect(img, 468, 178, 66, 3, 30);
      rect(img, 248, 210, 286, 8, 28);
    },
    min: 5,
    max: 5,
  },
  ...
  {
    name: "touching-horizontal-sticker-row",
    draw: (img) => {
      for (let col = 0; col < 4; col += 1) {
        const cx = 150 + col * 150;
        ellipse(img, cx, 170, 52, 64, 255);
        ellipse(img, cx + 54, 170, 30, 46, 244);
        ellipse(img, cx, 234, 48, 20, 242);
      }
    },
    min: 4,
    max: 4,
  ...
  {
    name: "five-touching-wide-stickers",
    draw: (img) => {
      for (let col = 0; col < 5; col += 1) {
        const cx = 128 + col * 125;
        ellipse(img, cx, 172, 60, 64, 255);
        ellipse(img, cx + 46, 172, 30, 44, 244);
        ellipse(img, cx, 236, 54, 18, 242);
      }
    },
    min: 5,
    max: 5,
  ...
  {
    name: "continuous-wide-banner-not-row-split",
    draw: (img) => {
      rect(img, 130, 160, 580, 86, 255);
      ellipse(img, 132, 203, 44, 43, 255);
      ellipse(img, 708, 203, 44, 43, 255);
      rect(img, 190, 184, 460, 22, 246);
    },
    min: 1,
    max: 1,
  },
  {
    name: "touching-two-row-sticker-grid",
    draw: (img) => {
      for (let row = 0; row < 2; row += 1) {
        for (let col = 0; col < 3; col += 1) {
          const cx = 190 + col * 150 + (row % 2) * 18;
          const cy = 135 + row * 116;
          ellipse(img, cx, cy, 52, 58, 255);
          ellipse(img, cx + 48, cy + 8, 25, 38, 244);
          ellipse(img, cx, cy + 58, 45, 18, 240);
        }
      }
    },
    min: 6,
  ...
  },
  {
    ...
    draw: (img) => {
      for (const cx of [185, 450, 715]) {
        ellipse(img, cx, 155, 58, 72, 255);
        rect(img, cx - 34, 218, 68, 54, 246);
        ellipse(img, cx, 286, 72, 18, 46);
      }
      rect(img, 96, 286, 708, 18, 34);
    },
    min: 3,
    max: 3,
  },
  {
    name: "continuous-subject-not-projection-split",
    draw: (img) => {
      rect(img, 245, 122, 410, 152, 255);
      ellipse(img, 278, 198, 72, 92, 255);
      ellipse(img, 622, 198, 72, 92, 255);
      rect(img, 330, 174, 240, 76, 255);
      rect(img, 420, 118, 60, 162, 250);
    },
    min: 1,
    max: 1,
  },
...
function projectionSplit(component, imageData, coreMask, minCoreArea, pad, settings) {
  ...
  const repeatedRow = splitRepeatedRowComponent(component, imageData, coreMask, minCoreArea, pad, settings);
  if (repeatedRow.length >= 2) return repeatedRow;
...
function splitByProjectionPeaks(component, imageData, coreMask, axis, minCoreArea, pad, settings) {
  ...
  const uniqueCuts = [...new Set(cuts)].filter((cut) => cut > length * 0.06 && cut < length * 0.94).sort((a, b) => a - b);
...
function splitStackedComponent(component, imageData, coreMask, minCoreArea, pad, settings) {
  ...
  for (let y = startY; y < endY; y += 1) {
    for (let x = startX; x < endX; x += 1) {
      if (coreMask[y * w + x]) projection[y - startY] += 1;
    }
  }
...
function splitRepeatedRowComponent(component, imageData, coreMask, minCoreArea, pad, settings) {
  if ((settings.mergeDistance ?? 4) > 2) return [];
  const aspect = component.width / Math.max(1, component.height);
  if (aspect < 2.8 || component.width < 160 || component.height < 24) return [];
  const { width: w, height: h } = imageData;
  const startX = Math.max(0, Math.floor(component.x));
  const startY = Math.max(0, Math.floor(component.y));
  const endX = Math.min(w, Math.ceil(component.x + component.width));
  ...
  for (let y = startY; y < endY; y += 1) {
    for (let x = startX; x < endX; x += 1) {
      if (coreMask[y * w + x]) projection[x - startX] += 1;
    }
  }
  const smoothed = smoothProjection(projection, Math.max(3, Math.round(length * 0.018)));
  const nonZero = smoothed.filter(Boolean);
  ...
  const average = nonZero.reduce((sum, value) => sum + value, 0) / nonZero.length;
  const max = Math.max(...smoothed);
  if (!max || average < component.height * 0.18) return [];
  const repeated = chooseRepeatedProjectionSplit(smoothed, aspect, 6);
  if (!repeated) return [];
  const { parts: estimatedParts, cuts } = repeated;
  const ranges = [];
  let last = 0;
  for (const cut of cuts) {
    ranges.push([last, cut]);
  ...
  ranges.push([last, length]);
  const children = componentsFromProjectionRanges(ranges, "x", { startX, startY, endX, endY }, imageData, coreMask, minCoreArea, pad, settings);
  if (children.length !== estimatedParts) return [];
  const parentArea = measureComponent(component, imageData, Math.max(24, settings.supportBase)).alphaArea;
  const childArea = children.reduce((sum, child) => sum + child.area, 0);
  const denseChildren = children.filter((child) => child.alphaDensity >= 0.18 && child.area >= minCoreArea).length;
...
function chooseRepeatedProjectionSplit(smoothed, aspect, maxParts) {
  const candidates = [...new Set([
    Math.ceil(aspect),
    Math.round(aspect),
    Math.floor(aspect),
  ].map((value) => clamp(value, 2, maxParts)))];
  let best = null;
  for (const parts of candidates) {
    const result = findRepeatedProjectionCuts(smoothed, parts);
    if (!result) continue;
    const score = result.balance * 0.72 + parts * 0.035 - result.averageValleyRatio * 0.24;
    if (!best || score > best.score) best = { ...result, score };
  }
  return best;
}

function findRepeatedProjectionCuts(smoothed, parts) {
  if (parts < 2) return null;
  const length = smoothed.length;
  const partSize = length / parts;
  const searchRadius = Math.max(12, Math.round(partSize * 0.32));
  const cuts = [];
  const valleyRatios = [];
  for (let part = 1; part < parts; part += 1) {
    const target = Math.round(partSize * part);
    let bestIndex = -1;
    let bestValue = Infinity;
    for (let cursor = Math.max(6, target - searchRadius); cursor <= Math.min(length - 6, target + searchRadius); cursor += 1) {
      const value = smoothed[cursor] ?? 0;
    ...
    if (bestIndex < 0) continue;
    const peakWindow = Math.max(8, Math.round(partSize * 0.28));
    const leftPeak = Math.max(...smoothed.slice(Math.max(0, bestIndex - peakWindow), bestIndex));
    const rightPeak = Math.max(...smoothed.slice(bestIndex + 1, Math.min(length, bestIndex + peakWindow + 1)));
    const localPeak = Math.min(leftPeak, rightPeak);
    const valleyRatio = localPeak > 0 ? bestValue / localPeak : 1;
    const farFromPrevious = !cuts.length || bestIndex - cuts[cuts.length - 1] >= partSize * 0.42;
    if (valleyRatio <= 0.74 && farFromPrevious) {
      cuts.push(bestIndex);
      valleyRatios.push(valleyRatio);
    }
  }
  if (cuts.length < parts - 1) return null;
  const ranges = [];
  let last = 0;
  for (const cut of cuts) {
    ranges.push(cut - last);
    last = cut;
  }
  ranges.push(length - last);
  const minRange = Math.min(...ranges);
  const maxRange = Math.max(...ranges);
  const balance = minRange / Math.max(1, maxRange);
  if (balance < 0.48) return null;
  return {
    parts,
    cuts,
    balance,
    averageValleyRatio: valleyRatios.reduce((sum, value) => sum + value, 0) / Math.max(1, valleyRatios.length),
  };
}
...
function measureComponent(component, imageData, alphaThreshold) {
  ...
  return {
    ...
    boxRatio: boxArea / Math.max(1, imageData.width * imageData.height),
    strongContentWidth,
    strongContentHeight,
...
function mergeNarrowSideFragments(components, imageData, minArea, settings) {
...
function strongContentBoundsExcludingRect(component, imageData, alphaThreshold, excludeRect) {
  ...
  const excludeEndX = Math.min(w, Math.ceil(excludeRect.x + excludeRect.width) + 1);
  const excludeEndY = Math.min(h, Math.ceil(excludeRect.y + excludeRect.height) + 1);
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
  let maxY = -Infinity;
